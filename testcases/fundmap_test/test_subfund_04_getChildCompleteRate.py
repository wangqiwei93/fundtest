            ...
            for data_city in range(0, len(db.select_db(sql_data))):
                for response_city in range(0, len(result.response.json()['data'])):
                    if db.select_db(sql_data)[data_city] == result.response.json()['data'][response_city]['city']:
                        assert (float(db.select_db(sql_data)[data_city]['2022年投资额']) - 1) <= float(
                            re.findall("\d+\.\d+", result.response.json()['data'][response_city]["sumMoneyTwo"])[0]) < (
                                       float(db.select_db(sql_data)[data_city]['2022年投资额']) + 5), result.error
                        assert (float(db.select_db(sql_data)[data_city]['2021年投资额']) - 1) <= float(
                        ...
                        assert (float(db.select_db(sql_data)[data_city]['占比']) - 1) <= float(
                            re.findall("\d+\.\d+", result.response.json()['data'][response_city]["rate"])[0]) < (
...
if __name__ == '__main__':
    pytest.main(["-v", "-s", "test_subfund_04_getChildCompleteRate.py"])
