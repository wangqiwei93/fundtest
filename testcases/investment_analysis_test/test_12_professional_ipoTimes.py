...
def step_1(year):
    logger.info("步骤1 ==>> 专精特新融资轮次：{}页".format(year))
...
class TestGetMother():
    ...
    def test_get_mother(self, tagname, except_result, except_code, except_msg):
        ...
        if result.code == 401:
            ...
            datalist = FundMj_Sql().professional_ipoTimes_sql()
            ...
            for i in range(len(reslist)):
                for l in range(len(datalist)):
                    if reslist[i]['financingRounds'] == datalist[l]['financing_rounds']:
                        assert reslist[i]['id'] == datalist[l]['procount'], reslist[i]['financingRounds']
...
if __name__ == '__main__':
    pytest.main(["-s", "-q", "test_12_professional_ipoTimes_api.py"])
