...
class TestGetMother():
    ...
    @pytest.mark.single
    @pytest.mark.parametrize("province, tag, except_result, except_code, except_msg",
                             api_test_fund_mj_data["professional_getMapOfCountry"])
    def test_get_mother(self, province, tag, except_result, except_code, except_msg):
        logger.info("*************** 开始执行用例 ***************")
        result = post_professional_getMapOfCountry(province, tag)
        step_1(tag)
        if result.code == 401:
            logger.info("***************token失效 结束执行用例 ***************")
            assert result.code == 200
        else:
            datalist = FundMj_Sql().professional_getMapOfCountry_sql()
            reslist = result.response.json()['data']  # 接口返回data数据
            for key, value in reslist.items():
                for i in range(len(datalist)):
                    if key == datalist[i]['project_province']:
                        assert value == datalist[i]['procount'], key
...
if __name__ == '__main__':
    pytest.main(["-s", "-q", "test_10_professional_getMapOfCountry_api.py"])
