...
@allure.epic("光荣榜项目列表")
@allure.feature("光荣榜项目列表")
class TestGetMother():
    @allure.story("用例--光荣榜项目列表")
    @allure.description("该用例是针对光荣榜项目列表接口的测试")
    ...
    @pytest.mark.single
    @pytest.mark.parametrize(" listname, page, limit, except_result, except_code, except_msg",
                             api_test_fund_mj_data["honour_getlist"])
    def test_get_mother(self, listname, page, limit, except_result, except_code, except_msg):
        logger.info("*************** 开始执行用例 ***************")
        result = post_honour_getlist(listname, page, limit)
        step_1(listname)
        if result.code == 401:
            logger.info("***************token失效 结束执行用例 ***************")
            assert result.code == 200
        else:
            datalist = FundMj_Sql().honour_getlist_sql()
...
if __name__ == '__main__':
    pytest.main(["-s", "-q", "test_15_honour_getlist.py"])
