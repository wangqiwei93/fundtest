...
import allure
import re
from operation.fundmapmain import get_fundOverviewChildEvolute_investStatus
from testcases.conftest import api_data, api_data_sql
from common.logger import logger
from common.mysql_operate import db


@allure.step("步骤1 ==>> 子基金-基金投资状态")
def step_1(year):
    logger.info("步骤1 ==>> 子基金-基金投资状态：{}".format(year))


@allure.severity(allure.severity_level.NORMAL)
@allure.epic("基金总览")
@allure.feature("子基金-基金投资状态")
class TestGetMother():
    @allure.story('用例--子基金-基金投资状态')
    @allure.description("该用例是针对子基金-基金投资状态的测试")
    ...
    @pytest.mark.parametrize("sql", api_data_sql["test_overview_investStatus_fundtype_1_sql"])
    ...
    def test_get_mother(self, year, areaId, areaName, isSelf, fundtype, except_result, except_code, except_msg, sql):
        logger.info("*************** 开始执行用例 ***************")
        result = get_fundOverviewChildEvolute_investStatus(year, areaId, areaName, isSelf, fundtype)
        step_1(year)
        if result.code == 401:
            ...
            for i in range(0, len(sql)):
                if year == sql[i][0] and fundtype == 1 and isSelf == sql[i][1]:
                    sql_data = sql[i][2]
                    break
                elif year == sql[i][0] and fundtype == 2 and isSelf == sql[i][1]:
                    sql_data = sql[i][3]
                    break
            """
                省市区县分布 金额添加断言
...
if __name__ == '__main__':
    pytest.main(["-v", "-s", "test_overview_07_fundOverviewChildEvolute_investStatus.py"])