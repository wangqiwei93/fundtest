...
@allure.step("步骤1 ==>> 库连群")
def step_1(year):
    logger.info("步骤1 ==>> 项目库产业链列表搜索项：{}".format(year))
...
class TestGetMother():
    ...
    @pytest.mark.single
    @pytest.mark.parametrize("labelcode, name, except_result, except_code, except_msg", ProJect_Sql().industry_code_3())
    def test_get_mother(self, labelcode, name, except_result, except_code, except_msg):
        logger.info("*************** 开始执行用例 ***************")
        time.sleep(0.5)
        result = get_projectlib_getgrojectlist_two(labelcode)
        step_1(name)
        if result.code == 401:
            ...
            if result.response.json()['data'] != None:
                ...
                data_sql = ProJect_Sql().project_list_sql_3(labelcode)
...
if __name__ == '__main__':
    pytest.main(["-s", "-q", "test_05_projectlib_getprojectlist.py"])
