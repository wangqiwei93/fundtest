...
tokeng = 'eyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiJqaW5rb25nIiwibG9naW5fdXNlcl9rZXkiOiJmNThiMjliMy0yMzFlLTQ4ZTMtODAwYS02ZjFjOTlkNDIwMTEifQ.QZkp4j3mwDTfk7fm-h0Eyvto6WM_IMmyxFI0NoG65vUer4VeSuVa-wbWQkw-oqhbFgi_As6kkuJ7pu7FSlSpeg'
...
# 母基金  省市县区域分布
def get_fundOverviewMotherEvolute_regionArrange(year, areaId, areaName, type):
    """
        获取产业基金分布
        :return: 自定义的关键字返回结果 result
        """
    ...
    res = fundmap.fundOverviewMotherEvolute_region_Arrange(year, areaId, areaName, type, headers=headerform)
    result.success = False
    print(res.json())
    try:
        result.error = "接口返回码是 【 {} 】, 返回信息：{} ".format(res.json()['code'], res.json()['message'])
...
# 基金投资状态  母基金
def get_fundOverviewMotherEvolute_investStatus(year, areaId, areaName, isSelf, type):
    """
        获取产业基金分布
        :return: 自定义的关键字返回结果 result
        """
    ...
    res = fundmap.fundOverviewMotherEvolute_invest_Status(year, areaId, areaName, isSelf, type, headers=headerform)
    result.success = False
    try:
        result.error = "接口返回码是 【 {} 】, 返回信息：{} ".format(res.json()['code'], res.json()['message'])
        result.msg = res.json()['message']
...
# 基金投资状态  子基金
def get_fundOverviewChildEvolute_investStatus(year, areaId, areaName, isSelf, type):
    """
        获取产业基金分布
        :return: 自定义的关键字返回结果 result
        """
    ...
    res = fundmap.fundOverviewChildEvolute_invest_Status(year, areaId, areaName, isSelf, type, headers=headerform)
    result.success = False
    try:
        result.error = "接口返回码是 【 {} 】, 返回信息：{} ".format(res.json()['code'], res.json()['message'])
        result.msg = res.json()['message']
...
# 子基金   省市县分布
def get_fundOverviewChildEvolute_regionArrange(year, areaId, areaName, type):
    """
        获取产业基金分布
        :return: 自定义的关键字返回结果 result
        """
    ...
    res = fundmap.fundOverviewChildEvolute_region_Arrange(year, areaId, areaName, type, headers=headerform)
    result.success = False
    print(res.json())
    try:
        result.error = "接口返回码是 【 {} 】, 返回信息：{} ".format(res.json()['code'], res.json()['message'])
...
# 子基金   各地子基金财政出资情况
def get_fundOverviewChildEvolute_fincInvest(year, areaId, areaName, isSelf, type):
    """
        获取产业基金分布
        :return: 自定义的关键字返回结果 result
        """
    ...
    res = fundmap.fundOverviewChildEvolute_finc_Invest(year, areaId, areaName, isSelf, type, headers=headerform)
    result.success = False
    # print(res.json())
    try:
        result.error = "接口返回码是 【 {} 】, 返回信息：{} ".format(res.json()['code'], res.json()['message'])
